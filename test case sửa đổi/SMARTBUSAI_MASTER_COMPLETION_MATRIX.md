...
- **B. Route visualization**: already fixed in a prior audit pass (see `route_visualization_forensic_audit.md`, in this same folder); re-confirmed this pass that the fabrication bug has not regressed and the 4-tier priority pipeline (real DB → curated estimate → honest origin/destination-only) is intact.
